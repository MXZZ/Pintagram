...
function parseAbsoluteUrl(absoluteUrl, locationObj, appBase) {
  ...
  locationObj.$$port = mint(parsedUrl.port) || DEFAULT_PORTS[parsedUrl.protocol] || null;
}
...
function $SnifferProvider() {
  this.$get = ['$window', '$document', function($window, $document) {
    var eventSupport = {},
        android =
          mint((/android (\d+)/.exec(lowercase(($window.navigator || {}).userAgent)) || [])[1]),
...
function dateFilter($locale) {
  ...
  function jsonStringToDate(string) {
    ...
    if (match = string.match(R_ISO8601_STR)) {
      ...
      if (match[9]) {
        tzHour = mint(match[9] + match[10]);
        tzMin = mint(match[9] + match[11]);
      }
      dateSetter.call(date, mint(match[1]), mint(match[2]) - 1, mint(match[3]));
      var h = mint(match[4]||0) - tzHour;
      var m = mint(match[5]||0) - tzMin;
      var s = mint(match[6]||0);
  ...
  return function(date, format, timezone) {
    ...
    if (isString(date)) {
      date = NUMBER_STRING.test(date) ? mint(date) : jsonStringToDate(date);
    }
